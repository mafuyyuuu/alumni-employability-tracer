        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {summaryCards.map(c => (
            <div key={c.label} className="bg-white rounded-2xl p-5" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              <p className="text-2xl font-black" style={{ color: '#2d6a4f' }}>{c.value}</p>
          ...
          {!loading && view === 0 && (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={byYear} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                ...
                <Bar dataKey="employed" name="Employed" fill="#2d6a4f" radius={[4, 4, 0, 0]} />
                <Bar dataKey="unemployed" name="Unemployed" fill="#d1fae5" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
          ...
          {!loading && view === 1 && (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={byCourse} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                ...
                <Tooltip formatter={v => `${v}%`} />
                <Bar dataKey="rate" name="Employment Rate" fill="#2d6a4f" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
          ...
          {!loading && view === 2 && (
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={byGender} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                ...
                <Tooltip formatter={v => `${v}%`} />
                <Legend iconType="circle" iconSize={8} />
                <Line type="monotone" dataKey="male" name="Male" stroke="#2d6a4f" strokeWidth={2.5} dot={{ r: 4, fill: '#2d6a4f', stroke: '#fff', strokeWidth: 2 }} />
                <Line type="monotone" dataKey="female" name="Female" stroke="#52b788" strokeWidth={2.5} dot={{ r: 4, fill: '#52b788', stroke: '#fff', strokeWidth: 2 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
